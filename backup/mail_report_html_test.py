...
for log_name_key in count_table:
    ...
    if count_table[log_name_key].get(Day1ago) is None:
        count_table[log_name_key][Day1ago] = 0
    # 计算总数和平均数
    print  count_table[log_name_key].values()
    sum_table[log_name_key] = sum(count_table[log_name_key].values())
    print sum_table[log_name_key] 
    average_table[log_name_key] = sum_table[log_name_key] / len(count_table[log_name_key])
    print len(count_table[log_name_key]),average_table[log_name_key]
    # 判断昨天入库正确性
    if count_table[log_name_key][Day1ago] >= int(average_table[log_name_key] * 0.6):
